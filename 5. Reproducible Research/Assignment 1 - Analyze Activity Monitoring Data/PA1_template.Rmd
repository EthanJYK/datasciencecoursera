...
Using given [dataset](https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2Factivity.zip),  write a report that answers the questions detailed below. 
<br><br>
<br><br>
...
- The mean and median of the total number of steps taken per day are `r mean_act` and `r median_act`.
<br><br>
<br><br>
...
- The 5-minute interval contains the maximum number of steps on average is `r max_int`.
<br><br>
<br><br>
...
- To imput missing values, I replaced each NA with the average numbers of steps taken in the same interval.

- There are `r NAs` NAs in the data. The mean and median total number of steps taken per day estimated are `r mean_ip` and `r median_ip`. These values are different from that of non-imputed data, and the mean of the plot has increased since all NAs become average values after replacing values.
<br><br>
<br><br>
...
- On weekdays, the plot shows that people walk a lot only in relatively earlier periods of the day, but on weekends, people tend to walk more than weekdays on average, and the steps are more evenly distributed.  
